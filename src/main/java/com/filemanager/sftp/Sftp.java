...
public class Sftp {
     ...
     * @return String[]
     */
    public List ls(String pathName){
        String currentDir = currentDir();
        if(!changeDir(pathName)){
            return null;
        };
        List<String[]> result = list(Filter.ALL);
     ...
     * @return String[]
     */
    public List lsFiles(String pathName){
        String currentDir = currentDir();
        if(!changeDir(pathName)){
            return null;
        };
        List result = list(Filter.FILE);
        if(!changeDir(currentDir)){
            return null;
        }
        return result;
     ...
     * @return String[]
     */
    public List lsDirs(String pathName){
        String currentDir = currentDir();
        if(!changeDir(pathName)){
            return null;
        };
        List result = list(Filter.DIR);
        if(!changeDir(currentDir)){
            return null;
        }
        return result;
     ...
     * @return boolean
     */
    public boolean exist(List list, String name){
     ...
     */
    @SuppressWarnings("unchecked")
    private List list(Filter filter){
        Vector<LsEntry> list = null;
        try {
        ...
        for(LsEntry entry : list){
            if(filter(entry, filter)){
                resultList.add(new String[]{entry.getFilename(),entry.getAttrs().toString()});
            }
        }
        return resultList;
    }
